...
<html>
	...
	<style type="text/css">
     	body, html,#allmap {width: 100%;height: 100%;hidden;margin:0;}
     	.close-img {position:absolute; right:15px; top:12px; cursor:pointer; width:20px; height:20px;}
   	</style>
	...
	<section>
		...
		<img id="mapImg" src="${mapImg}" />
 		...
 		<ul class="address-list1">
 			...
 			<li><a href="<%=contextPath%>/rental/match/detail?tHouseId=${item.tHouseId}&duration=${item.duration}&weixinId=${weixinId}"><i></i><span>${item.duration}分钟</span>${item.communityName} ${item.buildingNO}号${item.roomNumber}</a></li>
 			</c:forEach>
 		</ul>
