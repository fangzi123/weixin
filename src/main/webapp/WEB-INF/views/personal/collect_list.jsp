			...
			<c:forEach items="${resultList}" var="item">
   			<li>
   				<a href="<%=contextPath%>/rental/match/detail?tHouseId=${item.tHouseId}&weixinId=${weixinId}"><i></i>${item.communityName}
   					<div class="msg"><em>${item.layout}室${item.hallQuantity}厅${item.toiletQuantity}卫</em>/<em>${item.longPrice}元/月</em>/<em>${item.floor}楼 (共${item.totalFloor}楼)${item.buildingNo}号${item.roomNumber}</em></div>
 	  			</a>
 	  		</li>
 	  		</c:forEach>
 		</ul>
