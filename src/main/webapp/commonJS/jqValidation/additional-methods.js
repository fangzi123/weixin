...
jQuery.validator.addMethod("yearMonth", function(value, element, param) {
	return this.optional(element) || /^\d{4}-?((0[1-9])|(1[0-2]))$/i.test(value);
},jQuery.validator.messages.yearMonth);


jQuery.validator.addMethod("yearMonthRange", function(value, element, param){	
	var target = $(param);
	var endDate = target.val();
	if (this.settings.onfocusout) {
		target.unbind(".validate-dateRange").bind("blur.validate-dateRange", function() {	
	...
	if(value && endDate){
		return new Number(value) >= new Number(endDate);
	}
	return true;
}, jQuery.validator.messages.yearMonthRange);


jQuery.validator.addMethod("passwdConfirm", function(value, element, param) {
    var passwd1=$('#newPasswd').val();
	var passwd2=value;
	
	return passwd1==passwd2;
},jQuery.validator.messages.passwdConfirm);

jQuery.validator.addMethod("level6_item", function(value, element, param) {
	var level;
	if (param.value == '1') {
		level = $("input[name='"+param.field +"']").val();
	} else {
		var supplierId = $("input[name='"+param.field+"']").val();
		if (supplierId  == '') return false;
		level = supplierId.split('|')[2];
	}
	
	
    if ( level== '6'&& value=='') return false
    return true;
},jQuery.validator.messages.level6_item);

jQuery.validator.addMethod("pwdUsername", function(value, element, param) {
	
    return $("#login_form").validate().element($("#username")) ;
},jQuery.validator.messages.pwdUsername);

jQuery.validator.addMethod("validateUser",function(value, element, param){
	if ( this.optional(element) ) {
	...
	previous.originalMessage = this.settings.messages[element.name].validateUser;
	this.settings.messages[element.name].validateUser = previous.message;
	...
	$.ajax($.extend(true, {
		...
		success: function(response) {
			validator.settings.messages[element.name].validateUser = previous.originalMessage;
			...
			if ( valid ) {
				...
				var errors = {};
				var message = response || validator.defaultMessage( element, "validateUser" );
	...
	return "pending";
},jQuery.validator.messages.validateUser);

jQuery.validator.addMethod("validatePwd",function(value, element, param){
	...
	previous.originalMessage = this.settings.messages[element.name].validatePwd;
	this.settings.messages[element.name].validatePwd = previous.message;
	...
	$.ajax($.extend(true, {
		...
		success: function(response) {
			...
			validator.settings.messages[element.name].validatePwd = previous.originalMessage;
			var valid = response === true || response === "true";
			if ( valid ) {
				var submitted = validator.formSubmitted;
				validator.prepareElement(element);
				...
				var errors = {};
				var message = response || validator.defaultMessage( element, "validatePwd" );
				errors[element.name] = previous.message = $.isFunction(message) ? message(value) : message;
				validator.invalid[element.name] = true;
				validator.showErrors(errors);
				$("p[class*='help-block']").each(function() {
	...
	return "pending";
},jQuery.validator.messages.validatePwd);

//upload file type
jQuery.validator.addMethod("vaidateFileType",function(value, element, param){
	/*var fileTypeValue = param.fileType;
	var fileName = param.fileName;
	console.log(fileTypeValue);
	console.log(fileName);
	if (fileTypeValue=="item" && fileName.indexOf('DS_ITEM')<0){
        return false; 
   }
   if (fileTypeValue=="supplier" && fileName.indexOf('supplier.xlsx')<0){
        return false; 
   }
   if (fileTypeValue=="sort" && fileName.indexOf('DS_SORT')<0){
        return false; 
   }
   if (fileTypeValue=="brandFinal"  && fileName.indexOf('supplier_goods.xlsx')<0){
        return false; 
   }
   
   if (fileTypeValue=="sale" && fileName.indexOf('DS_SALE')<0){
        return false; 
   }
   */
   return true;
},jQuery.validator.messages.vaidateFileType);


jQuery.validator.addMethod("validateCaptcha",function(value, element, param){
	if ( this.optional(element) ) {
		return "dependency-mismatch";
	}
	...
	$.ajax($.extend(true, {
		...
		success: function(response) {
			...
			if ( valid ) {
				...
				var errors = {};
				var message = response || validator.defaultMessage( element, "validateCaptcha" );
				errors[element.name] = previous.message = $.isFunction(message) ? message(value) : message;
				validator.invalid[element.name] = true;
				validator.showErrors(errors);
				$("p[class*='help-block']").each(function() {
	...
	return "pending";
},jQuery.validator.messages.validateCaptcha);

jQuery.validator.addMethod("validateSupplier",function(value, element, param){
	...
	$.ajax($.extend(true, {
		...
		success: function(response) {
			...
			if ( valid ) {
				...
				var errors = {};
				var message = response || validator.defaultMessage( element, "validateSupplier" );
				errors[element.name] = previous.message = $.isFunction(message) ? message(value) : message;
				validator.invalid[element.name] = true;
				validator.showErrors(errors);
				$("p[class*='help-block']").each(function() {
	...
	return "pending";
},jQuery.validator.messages.validateSupplier);
