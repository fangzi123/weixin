...
public class WeixinPayController extends _BaseController {
    ...
    public ModelAndView payEntry(HttpServletRequest request,
        ...
        if (prepayId != null) {
            ...
            mv.addObject("activityId",findIntegerParameterValue(request,"activityId"));
            mv.addObject("openId",findStringParameterValue(request, "weixinId"));
        } else {
            mv.addObject("error", true);
    ...
    public ModelAndView payConfirm(HttpServletRequest request) {
        ...
        activityOrder.setActivityId(findIntegerParameterValue(request,"activityId"));
        activityOrder.setWeixinId(findStringParameterValue(request,"openId"));
        // 设定使用状态为“未使用”
