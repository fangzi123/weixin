...
public class RentalController extends _BaseController {
		...
		if (signature != null && checkSignature(signature, timestamp, nonce)) {// 从微信发起的请求
			if (echostr != null) {//token校验
				...
				if (msgHandler != null) {
					...
					return msgHandler.handleMessage(xmlMsg, getServerRootURL(request));
				} 
				return "请求有误";
			}
		}
	 ...
	 * @return 服务器根URL
	 */
	private String getServerRootURL(HttpServletRequest request) {
		String rootUrl = null;
		try {
			rootUrl = ConfigUtil.getConfig("balancer.server.url");
		} catch (Exception e) {
			log.error("取配置信息失败![balancer.server.url]");
